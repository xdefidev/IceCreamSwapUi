...
import { coreTokens, ICE } from '@pancakeswap/tokens';
...
var SUPPORTED_CHAIN_IDS = [ChainId.CORE, ChainId.NEON, ChainId.BITGERT, ChainId.XDC];
...
var POOLS_CONFIG_BY_CHAIN = {
  [ChainId.CORE]: pools,
  [ChainId.BITGERT]: pools2,
  [ChainId.XDC]: pools3,
  [ChainId.NEON]: pools4
};
var LIVE_POOLS_CONFIG_BY_CHAIN = {
  [ChainId.BSC]: livePools,
  [ChainId.BITGERT]: livePools2,
  [ChainId.XDC]: livePools3,
  [ChainId.NEON]: livePools4
};
...
var getPoolAprByTokenPerBlock = (stakingTokenPrice, rewardTokenPrice, totalStaked, tokenPerBlock, chainId) => {
  const totalRewardPricePerYear = new BigNumber7(rewardTokenPrice).times(tokenPerBlock).times(blocksPerYear(chainId));
...
var getNonBnbPools = getPoolsFactory((pool) => pool.stakingToken.symbol.toLowerCase() !== WETH9[pool.stakingToken.chainId].symbol.toLowerCase().substring(1));
var getBnbPools = getPoolsFactory((pool) => pool.stakingToken.symbol.toLowerCase() === WETH9[pool.stakingToken.chainId].symbol.toLowerCase().substring(1));
...
export { BOOST_WEIGHT, DURATION_FACTOR, ICE_FLEXIBLE_SIDE_VAULT, ICE_VAULT, IICE, LIVE_POOLS_CONFIG_BY_CHAIN, MAX_LOCK_DURATION, ONE_WEEK_DEFAULT, POOLS_CONFIG_BY_CHAIN, PoolCategory, SECONDS_IN_YEAR, SUPPORTED_CHAIN_IDS, UNLOCK_FREE_DURATION, VaultKey, blockTime, blocksPerYear, cakeFlexibleSideVaultV2ABI, cakeVaultV2ABI, fetchFlexibleSideVaultUser, fetchPoolsAllowance, fetchPoolsProfileRequirement, fetchPoolsStakingLimits, fetchPoolsStakingLimitsByBlock, fetchPoolsTimeLimits, fetchPoolsTotalStaking, fetchPublicFlexibleSideVaultData, fetchPublicIfoData, fetchPublicVaultData, fetchUserBalances, fetchUserIfoCredit, fetchUserPendingRewards, fetchUserStakeBalances, fetchVaultFees, fetchVaultUser, getCakeFlexibleSideVaultAddress, getCakeVaultAddress, getContractAddress, getIfoCreditAddressContract, getLivePoolsConfig, getPoolAprByTokenPerBlock, getPoolAprByTokenPerSecond, getPoolContractBySousId, getPoolsConfig, getSmartChefChefV2Contract, getSousChefBNBContract, getSousChefV2Contract, iCakeABI, isLegacyPool, isPoolsSupported, isUpgradedPool, smartChefABI, sousChefABI, sousChefBnbABI, sousChefV2ABI, sousChefV3ABI };
