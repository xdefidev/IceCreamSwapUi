...
import { getAddress, encodeFunctionData, decodeFunctionResult, parseUnits, encodePacked } from 'viem';
...
import { defaultChainId, getChain, chains } from '@icecreamswap/constants';
...
var SUCCESS_RATE_CONFIG = chains.reduce((acc, chain) => {
...
var _SwapRouter = class {
      ...
      if (singleHop) {
        ...
        for (let i = 0; i < sections.length; i++) {
          ...
          if (mixedRouteIsAllV3(newRoute)) {
            ...
            if (isExactIn) {
              const pools2 = newRoute.pools.map((pool) => pool.address ? pool.address : Pair.getAddress(pool.reserve0.currency.wrapped, pool.reserve1.currency.wrapped));
              const exactInputParams = [
                inAmount,
                // amountIn
                outAmount,
  ...
  static encodeSwaps(anyTrade, options, isSwapAndAdd) {
    ...
    const performAggregatedSlippageCheck = sampleTrade.tradeType === TradeType.EXACT_INPUT;
